...
# _install_prompt_first_user_name <default> — interactive prompt for the first
# user's name. A meaningful name (e.g. "home", "work-de", a hostname) makes it
# easy to tell servers apart in a client; the literal "default" everywhere does
# not. Empty input or cancel keeps <default>; invalid input re-prompts. Validated
# with users_validate_name. Only called in interactive mode when --name was absent.
_install_prompt_first_user_name() {
  local def="${1:-default}" entered
  while :; do
    entered="$(whiptail --inputbox \
      "Name for the first user.\n\nA label that tells this server apart in your client (e.g. 'home', 'work-de', or a hostname) — better than 'default' on every box.\n\nLetters, digits, _ or - (max 32):" \
      14 70 "$def" 3>&1 1>&2 2>&3)" || { printf '%s' "$def"; return 0; }
    [ -z "$entered" ] && entered="$def"
    if users_validate_name "$entered"; then
      printf '%s' "$entered"
      return 0
    fi
    whiptail --msgbox "Invalid name '${entered}'.\nUse letters, digits, _ or - (max 32 chars)." \
...
do_install() {
  ...
  local ip=""
  local name="default"
  local name_set=0   # 1 once --name is given, so interactive mode won't prompt.
  ...
  while [ "$#" -gt 0 ]; do
    case "$1" in
      ...
      --ip=*)          ip="${1#*=}"; shift ;;
      --name)          name="${2:?--name requires an argument}"; name_set=1; shift 2 ;;
  ...
  if _install_registry_empty; then
    # Interactive first install with no explicit --name: ask for a meaningful
    # name so multiple servers aren't all "default" (hard to tell apart by IP).
    if [ "$silent" != "1" ] && [ "$name_set" = "0" ]; then
      name="$(_install_prompt_first_user_name "$name")"
    fi
    if [ -n "$port" ]; then
