...
# touches SSH, so failures here are non-fatal to the mutation. The function
# itself is guarded with `command -v` (mirroring _users_service_installed): in
# the assembled bundle lib/firewall.sh is always present, but a slim test harness
# that sources only config/users/link must warn+skip rather than emit
# `firewall_open_port: command not found`.
_users_open_firewall() {
  local port="$1"
  if ! command -v firewall_open_port >/dev/null 2>&1; then
    log_warn "firewall support not loaded; skipping open of ${port}/tcp+udp."
    return 0
  fi
...
# Same `command -v` guard as _users_open_firewall (firewall.sh always present in
# the bundle; warn+skip when sourced standalone without it).
_users_close_firewall() {
  local port="$1"
  if ! command -v firewall_close_port >/dev/null 2>&1; then
    log_warn "firewall support not loaded; skipping close of ${port}/tcp+udp."
    return 0
  fi
